    ...
    <AnimatePresence>
      <motion.div key="backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div key="modal" initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
          ...
          style={{ background: '#1e293b', border: '1px solid #334155', boxShadow: '0 25px 80px rgba(0,0,0,0.7)' }}>
          ...
          <div className="flex flex-col items-center gap-1">
            <Logo size={64} shape="circle" className="mb-1" style={{ border: '3px solid rgba(37,99,235,0.4)', boxShadow: '0 0 24px rgba(37,99,235,0.3)' }} />
            <h2 className="font-bold text-slate-100 text-lg" style={{ fontFamily: 'Sora' }}>
              {tab === 'forgot' ? 'Reset Password' : 'Welcome to MyPy Tutor'}
            </h2>
            <p className="text-xs text-slate-500 text-center">Africa's AI Python &amp; ML Tutor</p>
          </div>

          {/* Tabs */}
          {tab !== 'forgot' && (
            <div className="flex rounded-2xl p-1 gap-1" style={{ background: '#0f172a' }}>
              {(['signin', 'signup'] as const).map(t => (
                <button key={t} onClick={() => { setTab(t); setError(''); setSuccess('') }}
                  className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${
                    tab === t ? 'text-white shadow-md' : 'text-slate-500 hover:text-slate-300'
                  }`}
                  style={tab === t ? { background: 'linear-gradient(135deg,#2563eb,#7c3aed)' } : {}}>
          ...
          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                ...
                {error.toLowerCase().includes('confirm') && (
                  <button onClick={handleResend} className="block mt-1.5 text-blue-400 text-xs hover:text-blue-300 transition-colors">
            ...
            {success && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
                className="rounded-xl px-4 py-3 text-sm text-green-300 border border-green-500/25"
            ...
            <form onSubmit={handleSignIn} className="flex flex-col gap-3">
              <div className="relative">
                ...
                <input type="email" placeholder="Email address" value={siEmail} onChange={e => setSiEmail(e.target.value)}
                  autoComplete="email" required className="pl-10 h-11" />
              </div>
              <div className="relative">
                ...
                <input type={siShow ? 'text' : 'password'} placeholder="Password" value={siPass} onChange={e => setSiPass(e.target.value)}
                  autoComplete="current-password" required className="pl-10 pr-10 h-11" />
              ...
              <button type="button" onClick={() => { setTab('forgot'); setFgEmail(siEmail); setError(''); setSuccess('') }}
                className="text-blue-400 text-xs hover:text-blue-300 transition-colors text-center">
                Forgot password?
              </button>
            </form>
          )}
            ...
            <form onSubmit={handleSignUp} className="flex flex-col gap-3">
              <div className="relative">
                ...
                <input type="text" placeholder="Full name" value={suName} onChange={e => setSuName(e.target.value)}
                  autoComplete="name" required className="pl-10 h-11" />
              </div>
              <div className="relative">
                <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-600 pointer-events-none" />
              ...
              <div className="relative">
                ...
                <input type={suShow ? 'text' : 'password'} placeholder="Password (min 8 chars)" value={suPass} onChange={e => setSuPass(e.target.value)}
                  autoComplete="new-password" required className="pl-10 pr-10 h-11" />
              ...
              <button type="submit" disabled={loading} className="btn btn-primary w-full h-11">
                {loading ? <><div className="loading-dots scale-75"><span/><span/><span/></div> Creating…</> : 'Create Account'}
              </button>
            </form>
          )}
          ...
          {tab === 'forgot' && (
            <form onSubmit={handleForgot} className="flex flex-col gap-3">
              ...
              <div className="relative">
                <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-600 pointer-events-none" />
                <input type="email" placeholder="Email address" value={fgEmail} onChange={e => setFgEmail(e.target.value)} required className="pl-10 h-11" />
              </div>
              <button type="submit" disabled={loading} className="btn btn-primary w-full h-11">
                {loading ? 'Sending…' : 'Send Reset Link'}
              </button>
              <button type="button" onClick={() => { setTab('signin'); setError(''); setSuccess('') }}
                className="flex items-center justify-center gap-1.5 text-blue-400 text-xs hover:text-blue-300 transition-colors">
          ...
          <div className="flex items-center gap-3 text-slate-600 text-xs">
            <div className="flex-1 h-px bg-slate-700/60" />or continue with<div className="flex-1 h-px bg-slate-700/60" />
          </div>

          {/* Google */}
          <a href={`${API_BASE}/auth/google/login`}
            className="flex items-center justify-center gap-3 w-full py-3 rounded-2xl border border-slate-700 bg-white text-slate-900 text-sm font-semibold hover:bg-slate-50 transition-all duration-150">
          ...
          <a href={`${API_BASE}/auth/github/login`}
            className="flex items-center justify-center gap-3 w-full py-3 rounded-2xl border border-slate-700 text-sm font-semibold transition-all duration-150 hover:border-slate-500"
            style={{ background: '#24292e', color: '#f0f6fc' }}>
          ...
          <p className="text-[10px] text-slate-600 text-center">By signing in you agree to our Terms of Service.</p>
        </motion.div>
      </motion.div>
    </AnimatePresence>
  )
