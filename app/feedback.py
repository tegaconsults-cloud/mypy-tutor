...
SURVEY_EVERY = 5
_feedback_loaded: bool = False


def _ensure_feedback_loaded() -> None:
    """Load persisted feedback from SQLite into memory on first access.
    This ensures get_summary() returns accurate data after a Render restart."""
    ...
    try:
        ...
        with _gdb() as _conn:
            for row in _conn.execute(
                "SELECT learner_id, rating, intent, topic, comment FROM feedback_ratings"
                " ORDER BY id ASC"
            ).fetchall():
            ...
            for row in _conn.execute(
                "SELECT learner_id, overall, clarity, helpfulness, suggestion, would_recommend"
                " FROM feedback_surveys ORDER BY id ASC"
            ).fetchall():
        ...
        logger.debug("Feedback DB load failed (non-fatal): %s", exc)
...
def record_message_feedback(fb: MessageFeedback) -> None:
    ...
    safe_lid     = _html.escape(fb.learner_id)
    safe_topic   = _html.escape(fb.topic or "—")
    safe_intent  = _html.escape(fb.intent or "—")
    safe_comment = _html.escape(fb.comment or "—")
...
<div style="font-family:Arial,sans-serif;max-width:560px;background:#1a202c;color:#e2e8f0;padding:24px;border-radius:12px;border:1px solid #2d3748;">
  ...
  <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
    ...
    <tr><td style="padding:6px 0;color:#718096;">Learner ID</td><td>{safe_lid}</td></tr>
    <tr><td style="padding:6px 0;color:#718096;">Topic</td><td>{safe_topic}</td></tr>
    <tr><td style="padding:6px 0;color:#718096;">Intent</td><td>{safe_intent}</td></tr>
    <tr><td style="padding:6px 0;color:#718096;">Comment</td><td>{safe_comment}</td></tr>
...
def record_survey(fb: SurveyFeedback) -> None:
    ...
    safe_lid        = _html.escape(fb.learner_id)
    safe_suggestion = _html.escape(fb.suggestion or "—")

    html = f"""
<div style="font-family:Arial,sans-serif;max-width:560px;background:#1a202c;color:#e2e8f0;padding:24px;border-radius:12px;border:1px solid #2d3748;">
  <h2 style="color:#63b3ed;margin-bottom:12px;">🐍 MyPy Tutor — Survey Response</h2>
  <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
    <tr><td style="padding:6px 0;color:#718096;width:160px;">Learner ID</td><td>{safe_lid}</td></tr>
    ...
    <tr><td style="padding:6px 0;color:#718096;vertical-align:top;">Suggestion</td>
        <td style="background:#0f1117;padding:10px;border-radius:6px;">{safe_suggestion}</td></tr>
    <tr><td style="padding:6px 0;color:#718096;">Time</td><td>{ts}</td></tr>
  </table>
</div>"""
    text = (
