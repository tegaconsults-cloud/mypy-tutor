"""
FastAPI application â€” MyPy Tutor (secured).
...
# Optional bearer dependency â€” declared here (module top) so it is available
...
# requiring authentication (auto_error=False means missing token â†’ None, not 401).
...
# ---------------------------------------------------------------------------
# Sentry error tracking â€” initialised early so all errors are captured.
# Set SENTRY_DSN in Render dashboard â†’ Environment to enable.
...
if _sentry_dsn:
    try:
        ...
        sentry_sdk.init(
            ...
            # (free tier has limits â€” keep this low)
        ...
        logger.warning("sentry-sdk not installed â€” run: pip install sentry-sdk[fastapi]")
...
try:
    import app.llm_client  # noqa: F401 â€” validates GROQ_API_KEY at startup
...
# GZip all responses â‰¥1KB â€” reduces bandwidth ~70% on Render's metered egress
...
async def chat(request: ChatRequest, req: Request,
    ...
    # Anonymous users (no token) pass freely â€” they use the free-tier quota.
    ...
    # â”€â”€ Resolve conversation_id â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Use client-provided conv_id if present; otherwise fall back to a
    # local synthetic ID. Do NOT call sb_get_or_create_conversation here â€”
    ...
    # â”€â”€ Build message list â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if not history_messages and sb_enabled() and request.conversation_id and not request.conversation_id.startswith("local_"):
        try:
            # Run in thread executor â€” sb_load_messages is synchronous (httpx/supabase-py)
            ...
            pass  # non-fatal â€” continue without history
    ...
    log_activity(request.learner_id, f"chat:{intent}",
                 f"topic={detected_topic or 'â€”'} | msg={request.message[:80]}")

    # â”€â”€ Persist: SQLite (sync, fast local) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Supabase writes are background tasks â€” never block the response â”€â”€â”€â”€
...
async def ping() -> dict:
    ...
    UptimeRobot setup:
      ...
      2. Add Monitor â†’ HTTP(s) â†’ URL: https://mypytutor.onrender.com/ping
      3. Check interval: 5 minutes (keeps Render free tier awake)
      4. Alert contacts: add your email / Telegram
    Returns 200 OK in <5ms â€” no DB or Supabase calls.
    """
    return {"ok": True}
...
async def auth_google_callback(code: str = None, error: str = None) -> JSONResponse:
    ...
    try:
        ...
        if not lp.email or not lp.display_name:
            ...
            _sp(lp)
        # Mirror to Supabase â€” non-blocking (fire-and-forget)
...
async def auth_google(request: GoogleAuthRequest) -> AuthResponse:
    """One-Tap / GSI token submitted directly from client â€” uses strict signature verification."""
...
# In-memory CSRF state store for GitHub OAuth.
# Entries expire after 10 minutes â€” long enough for any real browser flow.
import time as _time
_github_oauth_states: dict[str, float] = {}
_GITHUB_STATE_TTL = 600  # seconds
...
async def auth_github_login() -> JSONResponse:
    ...
    state  = _sec.token_hex(16)   # CSRF token â€” stored server-side for callback verification
    _github_oauth_states[state] = _time.time()
    params = urllib.parse.urlencode({
        "client_id":    client_id,
        "redirect_uri": redirect_uri,
...
async def auth_github_callback(code: str = None, error: str = None,
    ...
    """Handle GitHub OAuth callback â€” exchange code for token, fetch profile, sign in."""
    from fastapi.responses import RedirectResponse
    import urllib.parse, json
    ...
    # â”€â”€ CSRF state verification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Reject callbacks that carry an unknown or expired state token.
    now = _time.time()
    # Purge stale entries while we're here (keep the dict small)
    expired_keys = [k for k, ts in _github_oauth_states.items() if now - ts > _GITHUB_STATE_TTL]
    for k in expired_keys:
        _github_oauth_states.pop(k, None)

    if not state or state not in _github_oauth_states:
        logger.warning("GitHub OAuth: invalid or expired CSRF state from callback")
        return RedirectResponse(
            url=f"{frontend_url}/?auth=error&msg=Invalid+or+expired+sign-in+attempt.+Please+try+again."
        )
    # Consume the state â€” one-time use only
    del _github_oauth_states[state]
    ...
    try:
        ...
        if learner_id not in _users:
            _users[learner_id] = UserAccount(
                ...
                name=name,
                picture=picture,
                google_sub=github_id,   # repurposed field â€” stores GitHub id
            )
...
async def auth_signup(request: EmailSignUpWithCode) -> dict:
    ...
    If the code is invalid, signup still proceeds â€” we just skip the reward.
    """
    pw_hash = hash_password(request.password)

    # Validate code â€” check access_codes table first, then referrals
    ...
    if access_code:
        ...
        if code_rec:
            ...
            if referral_rec and referral_rec.get("uses", 0) < referral_rec.get("max_uses", 50):
                ...
                # Invalid code â€” don't block signup, just warn
                logger.info("Unrecognised code at signup: %s â€” proceeding without reward", access_code)
    ...
    # Store code info in pending â€” applied on email confirmation.
    ...
    if access_code and code_type == "access" and code_rec:
        if email_lower in _pending:
            ...
            # Auto-confirmed path â€” apply access code directly now
            try:
        ...
        if email_lower in _pending:
            ...
            # Auto-confirmed â€” record referral use directly
            try:
...
async def resend_confirmation(request: Request) -> dict:
    ...
    # Already confirmed â€” tell them to just sign in
    if email in _confirmed:
        return {"ok": True, "message": "Your email is already confirmed. Please sign in."}

    pending = _pending.get(email)
    if not pending:
        # Not pending and not confirmed â€” account doesn't exist or was lost on restart
        return {
            "ok":     True,
            "message": (
...
<body style="font-family:Arial,sans-serif;background:#0f1117;color:#e2e8f0;padding:32px;">
  <div style="max-width:520px;margin:0 auto;background:#1a202c;border-radius:16px;
    ...
    <h1 style="color:#63b3ed;font-size:1.4rem;margin-bottom:8px;">ðŸ MyPy Tutor</h1>
    ...
    <p style="color:#4a5568;font-size:0.75rem;">MyPy Tutor Â· Teamsamikoko Global Academy</p>
  </div>
</body>
</html>"""
    text_body = (
    ...
    # Check if email is actually configured â€” if not, auto-confirm instead
...
async def submit_enquiry(body: _EnquiryRequest) -> dict:
    """
    User support enquiry â€” forwarded to support@mypytutor.com.ng
...
async def get_certificate(
    ...
    # Admin preview: skip eligibility â€” admin token required
    if admin_view and request:
        try:
            _require_admin(request)
        except HTTPException:
            admin_view = False   # invalid token â€” fall through to normal check
    ...
    # allowed â€” they go through the normal eligibility check below.
    if user is not None:
        learner_id = user.learner_id   # always use session learner_id
        # Resolve display name from session user â€” ignore client-supplied ?name=
    ...
    if not admin_view and not tier_ok and not courses_ok:
        tier_names = {
            "basic":     "Beginner Bundle (â‚¦8,000) or complete all 4 beginner courses",
            "advanced":  "Intermediate Bundle (â‚¦15,000) or complete all 7 courses",
            "executive": "Elite Bundle (â‚¦35,000) or complete all advanced courses",
        }
        return HTMLResponse(
            ...
            <body><div class="box"><h2>ðŸ”’ Certificate Locked</h2>
            <p>To earn the <strong>{level.title()} Certificate</strong>, either:</p>
            <p>âœ… Purchase the <strong>{tier_names.get(level,'appropriate plan')}</strong></p>
            <p>â€” or â€”</p>
            <p>ðŸ“š Complete all required courses in this track</p>
            <a href="https://paystack.shop/pay/vt_re4d3h52" target="_blank">ðŸ’³ Upgrade Now</a>
            </div></body></html>""",
            status_code=402,
        )
    ...
    # Non-blocking Supabase write â€” cert HTML is generated immediately
...
# ---------------------------------------------------------------------------
# Certificate verification â€” public endpoint linked from cert emails
# ---------------------------------------------------------------------------
...
<title>Certificate Not Found â€” MyPy Tutor</title>
<style>
...
<html lang="en"><head><meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Certificate Verified â€” MyPy Tutor</title>
<style>
...
async def get_progress(learner_id: str,
    ...
    # We expose tier freely here because the frontend needs it for XP display â€”
    ...
    if credentials:
        try:
            ...
            if token_lid != learner_id:
                exposed_tier = ""   # different user â€” hide tier
        except Exception:
            exposed_tier = ""
    ...
    for name, meta in COURSE_CATALOG.items():
        ...
        if course:
            courses_detail.append({
                "name":         name,
                "display_name": course.description.split(" â€” ")[0] if " â€” " in course.description else name.replace("-", " ").title(),
...
async def start_course(learner_id: str, course_name: str,
    ...
    # Dynamic access check â€” driven by COURSE_CATALOG tier_unlocks
    from app.courses import COURSE_CATALOG
    meta = COURSE_CATALOG.get(course_name, {})
    ...
    if not individually_purchased and profile.tier not in allowed_tiers:
        ...
        tier_names = {"tier1": "Beginner Bundle (â‚¦8,000)", "tier2": "Intermediate Bundle (â‚¦15,000)", "tier3": "Elite Bundle (â‚¦35,000)"}
        return JSONResponse(status_code=402, content={
            ...
            "paystack_url":       "https://paystack.shop/pay/vt_re4d3h52",
            "message": (
                f"{badge} **{course.description.split(' â€” ')[0]}** costs â‚¦{price:,} "
...
async def next_course_step(learner_id: str,
    ...
    if step_idx >= len(course.steps):
        ...
        return {
            ...
            "content": f"ðŸŽ‰ Congratulations! You've completed **{course.name}**. You earned {XP_PROJECT} XP!",
        }
...
async def generate_quiz(request: QuizRequest, req: Request,
    ...
    # Anonymous (no token) quiz requests are still allowed â€” they use IP-based
...
async def evaluate_quiz_answer(request: QuizAnswerRequest,
    ...
    validate_topic(request.topic)
    # If authenticated, enforce learner_id matches token â€” prevents XP farming
...
async def generate_exercise(learner_id: str, topic: str,
    ...
    # Anonymous users (no token) are allowed â€” free-tier discovery via chat.
...
def _parse_quiz(raw: str) -> tuple[str, list[str]]:
    ...
    if not question:
        question = raw.split("\n")[0].strip()
    if not options:
        options = ["A) See full response", "B) â€”", "C) â€”", "D) â€”"]
    return question, options

# ---------------------------------------------------------------------------
# Paystack webhook â€” handles tier bundles, individual course purchases, prompt plans
# ---------------------------------------------------------------------------

# Map Paystack plan name (lowercase) â†’ internal tier
...
# Prompt plan names â†’ prompt tier key
...
async def paystack_webhook(request: Request) -> dict:
    ...
    1. Tier bundle purchase  â†’ upgrade learner.tier
    2. Individual course     â†’ record_course_purchase(learner_id, course_name)
    3. Prompt plan purchase  â†’ upgrade prompt daily limit (stored on learner profile)
    """
    secret_key = _os.getenv("PAYSTACK_SECRET_KEY", "")
    body_bytes  = await request.body()

    # â”€â”€ CRITICAL: always verify the HMAC signature.
    # If PAYSTACK_SECRET_KEY is not set we reject the request entirely â€”
    # accepting unsigned webhooks would let anyone fake a payment.
    if not secret_key:
        logger.error(
            "PAYSTACK_SECRET_KEY env var is not set â€” rejecting webhook. "
            "Add it to Render â†’ Environment immediately."
        )
    ...
    if not hmac.compare_digest(sig_header, expected):
        logger.warning("Paystack webhook signature mismatch â€” ignored")
    ...
    if event_type == "charge.success":
        ...
        # â”€â”€ IDEMPOTENCY: skip if this reference was already processed â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if reference:
            try:
                ...
                with _gdb_idem() as _idem_conn:
                    ...
                    if existing:
                        logger.info("Paystack webhook: duplicate reference %s â€” skipped", reference)
                        return {"ok": True}
        ...
        # â”€â”€ Determine payment type from metadata â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ TYPE 1: Individual course purchase â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if course_meta and course_meta in (c.name for c in get_all_courses()):
            ...
            # Paystack charge.success means the payment IS confirmed â€” mark it immediately
            ...
            log_activity(learner_id, "payment:course",
                         f"course={course_meta} | â‚¦{amount_ngn:.0f}")
            logger.info("Paystack webhook: course purchase %s for %s | invoice=%s",
                        course_meta, email, invoice_id)

        # â”€â”€ TYPE 2: Tier bundle purchase â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        else:
            tier = _PAYSTACK_PLAN_TIER.get(plan_meta)

            if not tier:
                # Infer tier from amount using canonical bundle prices:
                # Premium/Advanced â‚¦100,000 | Intermediate â‚¦60,000 | Beginner â‚¦30,000
                # Certificate fees: exec-cert â‚¦100k | adv-cert â‚¦60k | basic-cert â‚¦30k
                if amount_ngn >= 90000:
                    tier = "tier4"   # Premium Bundle â‚¦100,000 (also Executive Masters cert)
                elif amount_ngn >= 50000:
                    tier = "tier2"   # Intermediate Bundle â‚¦60,000 (also Advanced cert)
                elif amount_ngn >= 25000:
                    tier = "tier1"   # Beginner Bundle â‚¦30,000 (also Basic cert)

            # â”€â”€ TYPE 3: Prompt plan (parallel check) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if prompt_plan:
                ...
                try:
                    ...
                    pass  # non-fatal â€” in-memory reset still applied
                log_activity(learner_id, "payment:prompt_plan",
                             f"plan={prompt_plan} limit={daily_limit} | â‚¦{amount_ngn:.0f}")
            ...
            if tier:
                ...
                tier_labels = {
                    "tier1": "Beginner Bundle â€” â‚¦30,000 (4 courses)",
                    "tier2": "Intermediate Bundle â€” â‚¦60,000 (7 courses)",
                    "tier3": "Advanced Bundle â€” â‚¦100,000 (14 courses)",
                    "tier4": "Premium Bundle â€” â‚¦100,000 (all 16 courses)",
                }
                plan_label = tier_labels.get(tier, tier)
                import secrets as _sec
                payment    = add_payment(email, customer.get("name", email),
                                         amount_ngn, plan_label, "paystack")
                # Paystack charge.success = payment IS confirmed â€” mark immediately
                confirm_payment(payment.id)
                invoice_id = f"INV-{_sec.token_hex(5).upper()}"
                create_invoice_db(invoice_id, payment.id, learner_id, email,
                                  customer.get("name", email), plan_label, amount_ngn)
                # Non-blocking Supabase mirror â€” webhook must return 200 fast
                ...
                try:
                    ...
                    if ref_use:
                        ...
                        logger.info("Credited â‚¦%s referral bonus (15%%) for code %s", bonus, _ref_code)
                except Exception as rb_exc:
                    logger.debug("Referral bonus credit failed: %s", rb_exc)

                log_activity(learner_id, "payment:webhook",
                             f"tier={tier} | â‚¦{amount_ngn:.0f} | invoice={invoice_id}")
                logger.info("Paystack webhook: upgraded %s â†’ %s | â‚¦%.0f",
...
async def request_validation_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
    try:
        first  = exc.errors()[0]
        field  = " â†’ ".join(str(loc) for loc in first.get("loc", []) if loc != "body")
...
async def admin_dashboard(request: Request) -> dict:
    ...
    # â”€â”€ Pull everything directly from SQLite for real-time accuracy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    try:
        ...
        with _gdb() as _conn:
            ...
            # Active today â€” prompt counter from persistent table
...
async def admin_list_users(request: Request) -> dict:
    ...
    users_map: dict = {}   # learner_id â†’ user dict
...
# set-tier â€” uses apply_tier_upgrade for atomic memory+SQLite+Supabase consistency
...
async def admin_confirm_payment(payment_id: str, request: Request) -> dict:
    ...
    try:
        ...
        if row:
            ...
            _svc_adm(
                subject=f"Payment confirmed: â‚¦{float(row['amount']):,.0f} â€” {row['plan']}",
...
async def admin_team(request: Request) -> dict:
    """Return team members and tasks â€” both from SQLite (persistent)."""
    _require_admin(request)
...
<div style="max-width:520px;margin:0 auto;background:#0d1120;border-radius:16px;padding:32px;border:1px solid rgba(13,71,161,0.3);">
  ...
  <h3 style="color:#93c5fd;margin-bottom:8px;">ðŸŽ‰ Team Invitation</h3>
  ...
  <p style="color:#4d6080;font-size:.75rem;margin-top:28px;">
    MyPy Tutor Â· Teamsamikoko Global Academy Â· Powered by TeamTega Technologies Limited
  </p>
</div></body></html>"""
...
<div style="max-width:520px;margin:0 auto;background:#0d1120;border-radius:16px;padding:32px;border:1px solid rgba(13,71,161,0.3);">
  ...
  <h3 style="color:#fcd34d;margin-bottom:8px;">ðŸ“‹ New Task Assigned</h3>
  <div style="background:#111827;border-radius:10px;padding:16px;margin-bottom:16px;">
    ...
    {f'<p style="color:#4d6080;font-size:.78rem;margin:10px 0 0;">â° Due: <strong style=color:#e2e8f0>{body.due_date}</strong></p>' if body.due_date else ''}
  </div>
...
async def admin_referrals(request: Request) -> dict:
    ...
    return {
        ...
        "split_info":              "5% discount to referee Â· 15% bonus to referrer",
    }
...
async def admin_test_email(request: Request) -> dict:
    ...
    # â”€â”€ Email_FROM diagnostic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    config_status = {
        "RESEND_API_KEY": f"âœ… set ({len(resend_key)} chars)" if resend_key else "âŒ NOT SET",
        "EMAIL_FROM":     email_from if email_from else "âŒ NOT SET",
        "EMAIL_FROM_OK":  "âœ… valid format" if from_has_address else "âš ï¸ " + from_warning,
        "EMAIL_USER":     email_user if email_user else "âŒ NOT SET",
        "EMAIL_PASS":     f"âœ… set ({len(email_pass)} chars)" if email_pass else "âŒ NOT SET",
        "EMAIL_HOST":     email_host,
        "EMAIL_PORT":     email_port,
        "APP_URL":        app_url,
        "provider_chain": "Resend â†’ SMTP fallback",
    }
    ...
    # â”€â”€ 1. Test Resend (12s hard timeout, NO retry) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if resend_key:
            ...
            try:
                ...
                r = httpx.post(
                    ...
                    json={
                        ...
                        "to":       [to],
                        "subject":  "MyPy Tutor â€” Email Delivery Test",
                        "html":     test_html_full,
                        "text":     test_txt,
                    },
                    timeout=10,
    ...
    # If Resend succeeded, return immediately â€” no need to test SMTP
    ...
    # â”€â”€ 2. Test SMTP fallback (12s hard timeout) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _try_smtp():
        ...
        try:
            ...
            msg["Subject"] = "MyPy Tutor â€” Email Delivery Test"
            msg["From"]    = ef
            msg["To"]      = to
    ...
    if not smtp_q.empty():
        ...
        results["smtp"] = {"ok": False, "error": "Timed out after 12s â€” check EMAIL_HOST/PORT"}
    ...
    # â”€â”€ Both failed â€” return detailed per-provider errors â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if "auth failed" in smtp_err.lower():
        diagnosis.append("SMTP: Gmail App Password is wrong â€” re-create at "
...
async def generate_assignment(learner_id: str, topic: str,
    ...
    title = f"{topic} â€” Coding Assignment"
...
_LESSON_RESOURCES: dict[str, list[dict]] = {
    ...
    "Python Variables":              [{"type":"docs",    "label":"W3Schools Variables",               "url":"https://www.w3schools.com/python/python_variables.asp"},
                                      {"type":"article", "label":"Real Python â€” Variables",           "url":"https://realpython.com/python-variables/"}],
    ...
    "Python Strings":                [{"type":"docs",    "label":"W3Schools Strings",                 "url":"https://www.w3schools.com/python/python_strings.asp"},
                                      {"type":"article", "label":"Real Python â€” Strings",             "url":"https://realpython.com/python-strings/"}],
    ...
    "Python Functions":              [{"type":"docs",    "label":"W3Schools Functions",               "url":"https://www.w3schools.com/python/python_functions.asp"},
                                      {"type":"article", "label":"Real Python â€” Functions",           "url":"https://realpython.com/defining-your-own-python-function/"}],
    "Classes and Objects":           [{"type":"docs",    "label":"W3Schools OOP",                     "url":"https://www.w3schools.com/python/python_classes.asp"},
                                      {"type":"article", "label":"Real Python â€” OOP",                 "url":"https://realpython.com/python3-object-oriented-programming/"}],
    ...
    "Python RegEx":                  [{"type":"docs",    "label":"W3Schools RegEx",                   "url":"https://www.w3schools.com/python/python_regex.asp"},
                                      {"type":"tool",    "label":"Regex101 â€” Live tester",            "url":"https://regex101.com/"}],
...
_DEFAULT_RESOURCES = [
    ...
    {"type":"tool",  "label":"Python Tutor â€” Visualiser",     "url":"https://pythontutor.com/"},
]
...
# REFERRAL routes â€” specific paths BEFORE dynamic /{learner_id}
...
async def use_referral(body: ReferralUse,
    ...
    Requires auth â€” learner_id in body must match session token to prevent
...
async def get_invoice(invoice_id: str,
    ...
    """Return a printable invoice. Requires authentication â€” owner or admin only."""
    import re as _re4
...
<head>
  <meta charset="UTF-8"/>
  <title>Invoice #{inv['id']} â€” MyPy Tutor</title>
  <style>
...
<div class="invoice">
  <div class="hdr">
    <div>
      ...
      <p>Certified by Teamsamikoko Global Academy Â· Reg No: 3508656</p>
    </div>
  ...
  <div class="body">
    <div class="meta">
      ...
      <div style="text-align:right">
        <span class="status">âœ… PAID</span>
    ...
    <div class="line">
      <span>{inv['plan']}</span>
      <span style="font-weight:700">â‚¦{inv['amount']:,.0f}</span>
    </div>
    <hr/>
    <div class="total">
      <span>Total Paid</span>
      <span>â‚¦{inv['amount']:,.0f} {inv['currency']}</span>
    </div>
  ...
  <div class="ftr">
    <p>MyPy Tutor Â· mypytutor.onrender.com</p>
    <p style="margin-top:4px">TeamTega Technologies Limited Â· Teamsamikoko Global Academy</p>
    <p style="margin-top:4px;font-style:italic">"Learn Smarter. Code Better. Build the Future."</p>
  </div>
</div>
<button class="pbtn no-print" onclick="window.print()">ðŸ–¨ï¸ Print / Save as PDF</button>
</body>
</html>"""
  ...
  <tr><td style="background:#0d2b6e;padding:18px 36px;text-align:center;">
    <p style="color:rgba(255,255,255,0.75);font-size:0.73rem;margin:0;">
      MyPy Tutor &middot; Teamsamikoko Global Academy &middot; &ldquo;Learn Smarter. Code Better. Build the Future.&rdquo;
            ...
            _send_email_async(
                body.sent_to_email,
                f"Your MyPy Tutor Access Code â€” {tier_label}",
                html, text,
            )
            email_sent = True
        except Exception as exc:
...
# ---------------------------------------------------------------------------
# SUPABASE â€” Conversation & history routes
# ---------------------------------------------------------------------------
...
async def new_conversation(learner_id: str, background_tasks: BackgroundTasks,
                           user=Depends(get_current_user)) -> dict:
    """Start a fresh conversation. Owner-only â€” requires auth."""
    validate_learner_id(learner_id)
    if user is None:
    ...
    # Fire-and-forget Supabase insert â€” does NOT block the response
    if sb_enabled():
...
# ---------------------------------------------------------------------------
# SUPABASE â€” Status & health check
# ---------------------------------------------------------------------------

@app.get("/supabase/status")
async def supabase_status(request: Request) -> dict:
    """Check whether Supabase is configured and reachable. Admin-only â€” leaks infra URL."""
    _require_admin(request)
    if not sb_enabled():
    ...
    try:
        ...
        # Return generic error â€” do NOT expose internal connection details
        logger.warning("Supabase status check failed: %s", exc)
        return {"enabled": True, "status": "error",
                "detail": "Connection test failed â€” check Render logs for details."}


# ---------------------------------------------------------------------------
# STARTUP â€” Supabase data recovery on Render restart
# ---------------------------------------------------------------------------
...
def _recover_from_supabase() -> None:
    """
    CRITICAL STARTUP RECOVERY â€” runs on every boot.

    Render free tier wipes the filesystem on every deploy.
    This function:
    1. Pulls ALL email accounts from Supabase â†’ writes to SQLite + in-memory
    2. Pulls ALL learner progress from Supabase â†’ writes to SQLite
    ...
    if not sb_enabled():
        logger.info("Supabase not configured â€” skipping cloud recovery")
        return
    ...
    # â”€â”€ Step 1: Recover email accounts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Step 2: Recover learner progress â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Always restore from Supabase â€” Supabase is the source of truth.
    ...
    # â”€â”€ Step 3: Recover referral codes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# Run recovery in a background thread â€” never blocks the first incoming request.
...
async def get_profile_data(learner_id: str,
    ...
    # Base data â€” safe to return to anyone (no tier/email exposure to non-owners)
    result = {
        "learner_id":   learner_id,
...
async def update_profile(learner_id: str, body: UserProfileUpdate,
    ...
    # Sanitise photo_url â€” accept only https:// URLs or base64 data URLs for images.
    ...
    if photo:
        ...
        # Enforce size limit on base64 images (max 2MB decoded â‰ˆ ~2.7MB base64)
...
# ---------------------------------------------------------------------------
# Account deletion â€” NDPR/GDPR right to erasure (Article 17)
# ---------------------------------------------------------------------------

class _DeleteAccountRequest(_BM):
    password: str = _Field(..., min_length=1, max_length=128,
                           description="Current password â€” confirms the user's identity")
...
async def delete_account(body: _DeleteAccountRequest,
    ...
    NDPR/GDPR right to erasure â€” permanently removes PII, anonymises
    ...
    # OAuth users (Google/GitHub): no password to verify â€” identity already proved by session token
    ...
    try:
        ...
        pass  # Non-fatal â€” caches will expire naturally
...
# ---------------------------------------------------------------------------
# FIX: Invoice PDF generation (ReportLab â€” no weasyprint, free tier safe)
# ---------------------------------------------------------------------------

@app.get("/invoice/{invoice_id}/pdf")
async def get_invoice_pdf(invoice_id: str,
                          user=Depends(get_current_user)) -> JSONResponse:
    """Generate a downloadable PDF invoice. Requires auth â€” owner only."""
    import re as _re5
    ...
    try:
        ...
        # Header
        story.append(Paragraph("<b>ðŸ MyPy Tutor</b>", ParagraphStyle(
            "hdr", parent=styles["Title"], textColor=navy, fontSize=22, spaceAfter=4)))
        story.append(Paragraph(
            "Powered by TeamTega Technologies Limited<br/>"
            "Certified by Teamsamikoko Global Academy Â· Reg No: 3508656",
        ...
        raise HTTPException(status_code=503, detail="PDF generation not available â€” install reportlab.")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# FIX: Paystack metadata â€” store learner_id in payment so Google users
...
async def get_payment_metadata(learner_id: str,
    ...
    Requires auth â€” learner must be the owner to prevent email enumeration.
    """
    validate_learner_id(learner_id)
    if user is None:
...
# ---------------------------------------------------------------------------
# FIX: Render persistent disk â€” DB_PATH env var documented in render.yaml
#      (no code change needed â€” db.py already reads DB_PATH env var)
# ---------------------------------------------------------------------------


# ---------------------------------------------------------------------------
# Static assets â€” icons, manifest, certificates (NOT the full frontend app)
...
async def root() -> dict:
    """API root â€” confirms the backend is alive. Frontend is on Vercel."""
    return {
