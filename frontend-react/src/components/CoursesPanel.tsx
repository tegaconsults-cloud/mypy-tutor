...
import { BookOpen, Play, Lock, Clock, Layers, ArrowRight, Zap } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { getCatalog, getLearnerCourses } from '../api'
import { useAuth } from '../context/AuthContext'
import { useProgress } from '../context/ProgressContext'
import CoursePlayer from './CoursePlayer'

const CAT_ORDER = [
  'Python Basics', 'Intermediate Python', 'Advanced Python',
  'Data Science', 'Machine Learning', 'AI & Prompting',
]
...
export default function CoursesPanel() {
  ...
  // null = show catalog; non-null = playing a course
      ...
      try {
        ...
        if (user) {
          ...
          if (acc?.courses) {
            ...
            for (const c of acc.courses) {
              map[(c as { name: string }).name] = c as { unlocked: boolean; via: string }
            }
            setAccessMap(map)
          }
        }
  ...
  // Open the CoursePlayer in full focused mode
  const handleStart = (courseName: string, courseDisplayName: string, totalSteps: number) => {
    if (!user) return
    setActiveCourse({ name: courseName, title: courseDisplayName, totalSteps })
  }

  // Navigate to landing page IN-APP via React Router (no new tab)
  ...
  // ── If a course is active, render the player fullscreen ────────────
  if (activeCourse) {
    return (
      <CoursePlayer
        courseName={activeCourse.name}
        courseTitle={activeCourse.title}
        totalSteps={activeCourse.totalSteps}
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5 touch-scroll scrollbar-thin">
      ...
      {progress?.current_course && (() => {
        // Find display_name from catalog
        const allCourses = Object.values(catalog).flat()
        const current = allCourses.find(c => c.name === progress.current_course)
        const displayName = current?.display_name
          || progress.current_course.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
        const totalSteps = current?.total_steps || 10
        ...
        return (
          <motion.div
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <button
                onClick={() => handleStart(progress.current_course!, displayName, totalSteps)}
          ...
          <motion.div key={cat}
            ...
            <div className="flex flex-col gap-3">
              {items.map((course, idx) => {
                ...
                const step       = isActive
                  ? (progress?.current_course_step || 0)
                  : (isDone ? course.total_steps : 0)
                const pct        = course.total_steps > 0
                  ? Math.round((step / course.total_steps) * 100)
                  : 0
                  ...
                  <motion.div key={course.name}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 flex-wrap">
                          <div>
                            <h4 className="font-semibold text-sm text-white flex items-center gap-2 flex-wrap">
                              {course.display_name}
                              {isActive && !isDone && (
                                <span className="badge badge-blue text-[9px]">▶ Active</span>
                              )}
                              {isDone && (
                                <span className="badge badge-green text-[9px]">Done</span>
                              )}
                            </h4>
                            <p className="text-xs mt-0.5 line-clamp-2" style={{ color: '#4d6080' }}>
                              {course.description}
                            </p>
                          </div>
                        ...
                        <div className="flex items-center gap-3 mt-2 text-xs" style={{ color: '#4d6080' }}>
                          <span className="flex items-center gap-1">
                            <Layers size={10} />{course.total_steps} steps
                          </span>
                          <span className="flex items-center gap-1">
                            <Clock size={10} />{Math.ceil(course.total_steps * 5)} min
                          </span>
                          <span className="flex items-center gap-1">
                            <Zap size={10} style={{ color: '#E0A300' }} />{course.total_steps * 10} XP
                          </span>
                        </div>
                            ...
                            <div className="h-1.5 rounded-full overflow-hidden"
                              ...
                              <motion.div
                                ...
                                style={{
                                  background: isDone
                                    ? '#22c55e'
                                    : 'linear-gradient(90deg,#0D47A1,#E0A300)',
                                }} />
                            </div>
                          </div>
                        )}
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0 ml-1">
                        {isUnlocked ? (
                          <button
                            onClick={() => handleStart(course.name, course.display_name, course.total_steps)}
                          ...
                          <div className="flex flex-col items-end gap-1.5">
                            ...
                            {/* Learn More → in-app React Router navigation */}
                            {hasPage && (
                              <button
      ...
      {Object.keys(catalog).length === 0 && !loading && (
        <div className="flex-1 flex flex-col items-center justify-center py-16 gap-4">
          ...
          <p className="text-sm" style={{ color: '#4d6080' }}>
            Could not load course catalog. Please try refreshing.
          </p>
        </div>
      )}
    </div>
